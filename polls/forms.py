...
class gundemForm(forms.ModelForm):
	...
	gundem_tarihi = forms.DateField(required=True, widget=forms.DateInput(attrs={'class': 'form-control'}),label='Gündem Tarihi:')
	...
	#ekleyen_kullanici = forms.IntegerField(required=True, widget=forms.HiddenInput())
	tags = TagField(label="Etiketler", widget=TagWidget(attrs={'class':'form-control'}))
